...
{
    public static readonly string AppDataPath = GetAppDataName();
    ...
    {
        var finalFrontierEnvVar = Environment.GetEnvironmentVariable(BrandingConstants.AppDataEnvironmentVariable);
        if (!string.IsNullOrEmpty(finalFrontierEnvVar))
            return finalFrontierEnvVar;

        var envVar = Environment.GetEnvironmentVariable("SS14_LAUNCHER_APPDATA_NAME");
        if (!string.IsNullOrEmpty(envVar))
            return envVar;

        return BrandingConstants.AppDataName;
    }
}
